...
float GetCompletenessPurityIntersection(TTree* tree, int NLowerBound, int NUpperBound, std::string variable1, std::string variable2, std::string numberhits, std::string correctnessDefinition)
{
    std::cout << "*" << std::endl;
    int     MCForwards;
    ...
    tree->SetBranchAddress(correctnessDefinition.c_str(),                                             &MCIsClusterTwoParticles);
...
std::map<int, float> CreateCutsArray(TTree* t1, int nLowerBound, int nUpperBound, int nSteps, std::string variable1, std::string variable2, std::string numberhits, std::string correctnessDefinition)
{
    int nBinSize((nUpperBound - nLowerBound)/nSteps);
    std::map<int, float> cutValueMap;

    for (int i = 1; i <= nSteps; ++i)
        cutValueMap[i * nBinSize] = GetCompletenessPurityIntersection(t1, (i - 1) * nBinSize, i * nBinSize, variable1, variable2, numberhits, correctnessDefinition);

    cutValueMap[(nSteps + 1) * nBinSize] = GetCompletenessPurityIntersection(t1, nUpperBound, 1e6, variable1, variable2, numberhits, correctnessDefinition);
    return cutValueMap;
}

//------------------------------------------------------------------------------------------------------------------------------------------

std::map<int, float> CreateFailuresWithN(TTree* t1, int nLowerBound, int nUpperBound, int nSteps, std::string variable1, std::string variable2, std::string numberhits, std::string correctnessDefinition)
{
    int     MCForwards;
    int     MCIsClusterTwoParticles;
    int     NumberHits;
    ...
    t1->SetBranchAddress(correctnessDefinition.c_str(),                                             &MCIsClusterTwoParticles);
...
void Scatter(void)
{
    ...
    int removalRoutine(0); //0: track end filter, 1:splitting 2: fragment removal
    ...
    std::map<int, float> cutValueMap = CreateCutsArray(t1, 0, 400, 8, "SplittingBeforeChiSquaredPerHit", "SplittingAfterChiSquaredPerHit", "SplittingBeforeNumberHits", "MCIsClusterTwoParticles");
    ...
    std::map<int, float> failureCounterWithN = CreateFailuresWithN(t1, 0, 400, 8, "SplittingBeforeChiSquaredPerHit", "SplittingAfterChiSquaredPerHit", "SplittingBeforeNumberHits", "MCIsClusterTwoParticles");
